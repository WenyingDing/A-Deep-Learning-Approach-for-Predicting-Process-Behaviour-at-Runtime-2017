...
<xsl:template match="xes:event">
  <xsl:value-of select="translate(xes:string[@key='concept:name']/@value, '_-. ', '')"/><xsl:text>-</xsl:text><xsl:value-of select="translate(xes:string[@key='lifecycle:transition']/@value, '_-. ', '')"/><xsl:text>-</xsl:text><xsl:value-of select="translate(xes:string[@key='org:resource']/@value, '_-. ', '')"/>
  <xsl:text> </xsl:text>
</xsl:template>
